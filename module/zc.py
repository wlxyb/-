...
class Ui_ZC(QtWidgets.QMainWindow):
    ...
    def back_out(self):
        ...
        if self.pwText_2.text() == self.pwText.text():
            QtWidgets.QMessageBox.information(self, "提示", "设置成功，请返回登录",
            ...
            QtWidgets.QMessageBox.information(self, "提示", "两次密码不一致，请重新输入",
                                              QtWidgets.QMessageBox.Yes)
            pa += "两次密码不一致，请重新输入"
            self.textBrowser_2.append(pa)
            self.pwText.clear()
