...
def create_history(conn):
    ...
    record text,
    time timestamp default now(),
    people int not null)character set utf8;'''
    cur = conn.cursor()
    try:
        cur.execute(s)
        conn.commit()
...
def insert_history(rootname, f_rootname, record,people):
    conn, cur = creatConnect()
    print("history:",rootname, f_rootname)
    s = "insert into history(rootname, f_rootname,record,people) values('%s', '%s', '%s','%s')" % (
        rootname, f_rootname, record,people)
    try:
        cur.execute(s)
        conn.commit()
...
def create_group(conn):
    s = '''create table groupx(id int auto_increment primary key,group_number int,group_name varchar(20),group_manager int,group_avatar text)character set utf8;'''
    cur = conn.cursor()
    try:
        cur.execute(s)
        conn.commit()
        print('create_group is ok')
    except:
        print("create_group FILL")
        conn.rollback()


def select_group(group_number):
    conn, cur = creatConnect()
    s = "select group_number,group_name,group_avatar from groupx where group_number='%s'" % group_number
    cur.execute(s)
    data = cur.fetchone()
    close(conn, cur)
    return data
...
#创建一个群
def insert_group(rootname, group_id,group_name,group_avatar):
    conn, cur = creatConnect()
    s = 'insert into groupx(group_number,group_name,group_manager,group_avatar) values("%s","%s","%s","%s")' % (
        group_id, group_name,rootname,group_avatar)
    try:
        cur.execute(s)
        conn.commit()
        print("insert_group is ok")
    except Exception as e:
        print(e)
        conn.rollback()
    close(conn, cur)
...
# 群成员关系表
def create_group_list(conn):
    s = '''create table group_list(
        id int auto_increment primary key,
        group_number int not null,
        g_rootname int not null)character set utf8;'''
    cur = conn.cursor()
    try:
        cur.execute(s)
        conn.commit()
        print('create_group_list is ok')
    except:
        print("create_group_list FILL")
        conn.rollback()


def insert_group_list(group_number, g_rootname):
    conn, cur = creatConnect()
    s = 'insert into group_list(group_number,g_rootname) values("%s","%s")' % (group_number, g_rootname)
    try:
        cur.execute(s)
        conn.commit()
        print("insert_group_list is ok")
    except Exception as e:
        print(e)
        conn.rollback()
    close(conn, cur)
...
def delete_group(group_number):
    conn, cur = creatConnect()
    s = 'delete from groupx where group_number="%s' % group_number
    try:
        cur.execute(s)
        conn.commit()
        print("delete_group is ok")
    except Exception as e:
        print(e)
        conn.rollback()
    close(conn, cur)
...
def delete_group_list(group_number,g_rootname):
    conn, cur = creatConnect()
    s = 'delete from group_list where g_rootname="%s and group_number ="%s ' % (g_rootname,group_number)
    try:
        cur.execute(s)
        conn.commit()
        print("delete_group_list is ok")
    except Exception as e:
        print(e)
        conn.rollback()
    close(conn, cur)
...
def select_group_user(group_number,rootname):
    conn, cur = creatConnect()
    s = "select groupx.group_number,groupx.group_name,groupx.group_avatar from groupx,group_list where groupx.group_number='%s' and groupx.group_number=group_list.group_number and group_list.g_rootname='%s' " % (group_number,rootname)
    cur.execute(s)
    data = cur.fetchone()
    close(conn, cur)
    return data

def select_group_manager(group_number):
    conn, cur = creatConnect()
    s = "select groupx.group_number,groupx.group_name,groupx.group_manager,users.tel,users.address,users.email,users.birthday,groupx.group_avatar,groupx.group_manager,users.onlinestatus from users,groupx where users.rootname=groupx.group_manager and groupx.group_number='%s';" % (group_number)
    cur.execute(s)
    data = cur.fetchone()
    close(conn, cur)
    return data
...
# 查用户拥有的群
def select_group_count(rootname):
    conn, cur = creatConnect()
    s = "select group_number,group_name,group_manager,group_avatar from groupx where group_manager='%s';" %rootname
    cur.execute(s)
    data = cur.fetchall()
    close(conn, cur)
    return data
...
def select_groups(rootname):
    conn, cur = creatConnect()
    s = "select groupx.group_name,groupx.group_number,groupx.group_avatar from groupx,group_list where g_rootname='%s' and groupx.group_number=group_list.group_number" % rootname
    cur.execute(s)
    data = cur.fetchall()
    close(conn, cur)
    return data
...
# 查群成员
def select_group_list(group_number):
    conn, cur = creatConnect()
    s = "select users.nickname, users.rootname,users.gender,groupx.group_name,groupx.group_number from groupx,group_list,users where group_list.group_number='%s' and group_list.group_number= groupx.group_number and users.rootname=group_list.g_rootname;" % (group_number)
    cur.execute(s)
    data = cur.fetchall()
    close(conn, cur)
    return data
