from module.group_select import Ui_Group
from module.weather_query import Ui_Weather
...
class Mychat(QMainWindow, Ui_ChatForm):
    def __init__(self, my_info, s, friend_list):
        ...
        self.pushButton_4.clicked.connect(self.play_flie)
        self.pushButton_2.clicked.connect(self.play_group)
        self.pushButton.clicked.connect(self.play_jiahaoyou)

        self.pushButton_7.clicked.connect(self.show_item)

        self.pushButton_5.clicked.connect(self.play_xinxi)

        self.pushButton_3.clicked.connect(self.weather)
    ...
    def weather(self):
        self.wea = Ui_Weather(self.my_info, self.s)
        self.wea.show()

    def play_group(self):
        self.gro = Ui_Group(self.my_info, self.s)
        self.gro.show()
    ...
    def play_xinxi(self):
        os.system("python ./view/chengyuan.py")
    ...
    def chat3(self):
        self.friend_info = dict(zip(
            ["type", "rootname", "nickname", "gender", "tel", "address", "email", "birthday", "avatar", "style",
             "onlinestatus"],
            ["I", "1000", "小Root", "女", '861008610010', 'AID1904三组', '10086@10010.com', '1月1日', 'images/avatar0.jpg',
             'Hello Python', '在线']))
    ...
    def Recv(self):  # 接受消息
        while True:
            ...
            # 历史记录
                ...
                print("执行到了type=J")
                self.wea.recvwea(data["info"])
                self.s.send(("P " + self.my_info['rootname']).encode())

            elif data["type"] == "K":
                print("执行到了type=K")
                self.wea.recwea1(data["info"])
                self.s.send(("P " + self.my_info['rootname']).encode())

            elif data["type"] == "U":
                print("执行到了type=L")
                self.gro.recvinfo(data["info"])
                self.s.send(("P " + self.my_info['rootname']).encode())

            elif data["type"] == "B":
                print("执行到了type=B")
                self.gro.recvinfo1(data["info"])
                self.s.send(("P " + self.my_info['rootname']).encode())

            elif data["type"] == "N":
                print("执行到了type=N")
                self.gro.recvinfo2(data["info"])
                self.s.send(("P " + self.my_info['rootname']).encode())
